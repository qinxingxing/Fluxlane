...
/**
 * Prerendered public pages (except `/` and `404.html`) are emitted as
 * `dist/<path>.html`. Cloudflare Pages Pretty URLs then serve `/about`
 * from `about.html` with a 200. Emitting `about/index.html` instead makes
 * Pages 308 `/about` → `/about/`, which fights the slash-stripping rules
 * below and 308-loops forever.
 */
function publicPageHtmlPath(routePath: string): string {
...
function validateOutput(renderedRoutes: string[]): void {
  ...
  for (const route of renderedRoutes) {
    ...
    const file = publicPageHtmlPath(route)
...
async function main(): Promise<void> {
  ...
  for (const [routePath, routeSeo] of Object.entries(ROUTES)) {
    const outputPath = publicPageHtmlPath(routePath)
