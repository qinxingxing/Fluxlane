      ...
      <main className='relative overflow-hidden pt-16'>
        ...
        <section className='relative mx-auto flex max-w-7xl flex-col items-center px-6 py-20 text-center md:py-28'>
          ...
          <div className='relative mt-16 w-full max-w-5xl overflow-hidden rounded-2xl border border-white/10 bg-[#070b28]/90 text-left shadow-[0_0_40px_rgba(0,210,253,0.10)]'>
            <div className='flex h-11 items-center justify-between border-b border-white/10 bg-[#191d3b] px-4'>
              ...
              <span className='font-mono text-[10px] tracking-widest text-[#958da1] uppercase'>
                API Gateway · Live
              </span>
              ...
              <pre className='overflow-x-auto p-6 font-mono text-xs leading-7 text-[#ccc3d7] md:text-sm'>
                <code>
                  ...
                  <span className='text-amber-200'>
                    &quot;Authorization: Bearer $FLUXLANE_API_KEY&quot;
                  </span>
                  {'\n'} -d{' '}
              ...
              <div className='flex flex-col justify-center gap-3 border-t border-white/10 bg-[#141936]/70 p-6 md:border-t-0 md:border-l'>
                <div className='flex items-center justify-between'>
                  ...
                  <span className='flex items-center gap-2 font-mono text-xs text-emerald-300'>
                    <CheckCircle2 className='size-4' /> 200 OK
                  </span>
                </div>
                <div className='flex items-center justify-between'>
                  <span className='font-mono text-xs text-[#958da1]'>
          ...
          <div className='mx-auto grid max-w-7xl grid-cols-2 gap-8 px-6 py-11 md:grid-cols-4'>
            {stats.map(([value, label], index) => (
              <div key={label} className='text-center'>
                <p
                  className={`text-3xl font-bold md:text-4xl ${index % 2 === 0 ? 'text-violet-300' : 'text-cyan-300'}`}
                >
                  {value}
                </p>
                <p className='mt-2 font-mono text-[10px] tracking-widest text-[#958da1] uppercase'>
                  {label}
                </p>
              </div>
            ))}
        ...
        >
          <div className='mb-14 text-center'>
            <p className='font-mono text-xs font-semibold tracking-widest text-cyan-300 uppercase'>
              Core Features
            </p>
          ...
          <div className='grid gap-6 md:grid-cols-3'>
            {features.map(({ icon: Icon, title, description, color }) => (
              <article
                key={title}
                className='group relative overflow-hidden rounded-2xl border border-white/10 bg-[#161b33] p-8 transition hover:-translate-y-1 hover:border-violet-400/50 hover:shadow-[0_0_28px_rgba(109,40,217,0.18)]'
              >
                ...
                <p className='relative mt-3 text-sm leading-relaxed text-[#ccc3d7]'>
                  {description}
                </p>
              </article>
            ))}
          ...
          <div className='rounded-2xl border border-white/10 bg-[#191d3b]/70 p-8 md:p-12'>
            <div className='grid items-center gap-10 md:grid-cols-2'>
              <div>
                ...
                <p className='mt-4 text-[#ccc3d7]'>
                  创建账号和 API Key，选择适合的模型，将现有 OpenAI SDK
                  端点替换为 Fluxlane。
                </p>
              </div>
              <div className='space-y-4'>
                {[
                  ...
                  <div
                    key={step}
                    className='flex items-center gap-4 rounded-xl border border-white/10 bg-[#070b28]/60 p-4'
                  >
