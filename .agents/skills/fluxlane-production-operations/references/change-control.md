...
Keep the node out → stop the new version → restore the recorded image/commit/config → start → require readiness → validate → rejoin only if stable. If rollback fails, keep it out, preserve evidence, output `ROLLBACK FAILED`, end `BLOCKED`.

Prefer rollback over degraded continuation.
