...
- Pre-probe images serve neither `/readyz` nor `/healthz`: legacy rollback validates `/api/status`, uses the per-role Compose override (literal service keys, since Compose does not substitute variables in mapping keys), and swaps a **complete** Nginx site at `/etc/nginx/sites-available/fluxlane-api` (API) or `fluxlane-api-run` (RUN) after backing up the real file content plus `nginx -t`. A normal deploy restores the role template and verifies native `/readyz` before rejoin. Container healthcheck `none` fails closed.
