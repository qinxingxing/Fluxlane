#!/usr/bin/env bash
# Roll THIS RUN node back to a previously released tag using the saved artifact.
# Never rebuilds from source. Never runs `docker compose down`. The node must be
# detached from CLB and drained (Streaming/SSE at least 120s); the peer RUN node
# must stay healthy.
#
# Usage:
...
mkdir -p "$RECORD_DIR"
record_current_state "$CONTAINER" "$RECORD_DIR/run-rollback-$TARGET_TAG.before"
