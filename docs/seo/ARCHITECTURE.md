...
- Public mode prerenders `/`, `/about`, `/pricing`, `/privacy-policy`, `/faq`, `/user-agreement` into `dist/index.html` and `dist/<path>.html` (not `dist/<path>/index.html`). Cloudflare Pages directory indexes 308 to a trailing slash; slash-stripping `_redirects` then loop. Pretty URLs on `about.html` keep the slashless canonical.
