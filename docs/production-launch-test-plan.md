...
### 10.5 B17 实测结论 — 已接受风险（Accepted-Risk / 有条件 Go）

2026-08-25 在生产 CLB（`run.fluxlane.ai`）对测试账号 `qinxingxing-test`、`qa-mock-model` 实测。**穿透存在，但在预消耗配额 200000 下残留敞口 <$1/次爆发**，经风控确认为**已知可接受风险**，判定 **有条件 Go**（不再作为 No-Go）。完整数据见 `docs/testing-handoff/b17-overdraft-report.md`。

**参数**：单价 输入 $0.2/M、输出 $0.4/M；预消耗配额 200000 → 单次预扣 ≈ $0.04；为触发穿透机制，令 Mock 实报 200000/100000 → 单次实扣 $0.08 > 预扣；每轮钱包重置 $0.20（够 5 次预扣）。

**实测（单机并发打 CLB）**：

| 并发 | 抢过预扣(HTTP 200) | 结算后钱包 | 透支 |
| --- | ---: | ---: | ---: |
| 20 | 8 | −$0.44 | $0.44 |
| 50 | 7 | −$0.36 | $0.36 |
| 100 | 5 | −$0.20 | $0.20 |
...
- 抢过笔数被**共享 Redis 异步扣减的时间窗**封顶在 ~5–8，**不随并发增长**（20→8、200→6）。
- 透支深度 ≈ 抢过笔数 × 单次实扣 − 钱包；随**单次成本线性放大**。
- 钱包被打负后后续预扣立即 403，**同一次充值只能被穿一次**。

**根因**：预扣「读余额 `GetUserQuota`（共享 Redis）→ 判断 `余额−预扣<0` → 扣减 `DecreaseUserQuota`」非原子，且 Redis 扣减为 `gopool.Go` 异步 fire-and-forget；钱包扣减走无下限 `quota - N`，无 `WHERE quota >= N`、无行锁。

**CLB 不放大**：余额存同一份共享 Redis 哈希，两台 RUN 非「各扣各的」；抢过笔数随并发收敛而非发散；敞口由异步扣减窗口决定，与节点数无关。局限：单机发压，~8 为保守下限；单节点 vs CLB 铁证需从跳板机内网直连单台 RUN 对比（本次未做）。

**风险口径（缓解依据）**：

- 预消耗配额 **2000** 时预扣≈$0.0004，与贵价实扣差额巨大 → 单次爆发最大穿透 **~$40**。
- 预消耗配额 **200000** 时预扣≈$0.04，接近多数请求真实成本 → 残留穿透 **<$1**。

**采纳该风险的前提**：

- **不得调小预消耗配额**；任何调小都会重新放大透支，需回退到 No-Go 口径。
- 敞口随**单次实扣**线性放大，贵价模型需单独关注。
- 可选彻底加固：预扣改为原子条件扣减 `UPDATE ... SET quota = quota - N WHERE quota >= N`（受影响行数=0 即余额不足 403），消除穿透后可恢复「余额不穿透」为硬门禁。
...
- B1/B2/B5/B13 通过
- B17：Provider 调用 ≤ 允许次数、无残留预扣、无先调后发现没钱；余额穿透在预消耗配额 200000 下 <$1/次爆发，作为**已接受风险**（见 §10.5），**前提是预消耗配额保持 ≥ 200000**
...
- 只测了 `--resolve` 单节点
- B17 出现 200 次 > N、失败请求仍占额度、或渠道日志显示未预扣就调了上游
- **预消耗配额被调小到 < 200000**，使单次爆发穿透回升到数十美元级（§10.5 的接受前提被破坏）
- 两台 RUN 额度不一致
