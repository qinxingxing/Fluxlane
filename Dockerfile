...
RUN DISABLE_ESLINT_PLUGIN='true' VITE_API_BASE_URL="${VITE_API_BASE_URL}" VITE_PUBLIC_API_BASE_URL="${VITE_PUBLIC_API_BASE_URL}" VITE_REACT_APP_VERSION=$(cat /build/VERSION) bun run build
...
# GIT_COMMIT is the full source commit. Release builds must pass it so
# /api/status can identify the running code; local builds may leave it unset.
